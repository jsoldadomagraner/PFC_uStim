...
% FAdata0 contains the data from the whole trial period, which is used to 
% plot the entire trial trajectory in no uStim conditions (without excuding 
% the uStim period)
load([FApath FAmodel],'FAdata','FAdata0')
...
[postMeans0, ~] = fastfa_estep(y0,FAparams(numdims));
[~,postMeans0]  = orthonormalizeFAMdl(params,postMeans0.mean');
postMeans0      = postMeans0'; %sorted by decreasing shared var

% mark zero in latent space
v0 = zeros(size(y,1),1);
[postMeansv0, ~]  = fastfa_estep(v0,FAparams(numdims));
[~,postMeansv0]   = orthonormalizeFAMdl(params,postMeansv0.mean');
v0 = postMeansv0'; %sorted by decreasing shared var

ln  = size(postMeans,1);
xm  = reshape(postMeans,ln,ti,tr,co);
xm0 = reshape(postMeans0,ln,ti0,tr,co);
...
meand   = [3,4]; % average across trials for mean trajectory

c = 1;
xmang  = cell(1,4); xmang(:) = {zeros(ln,ti,tr,length(uStime))};
xmang0 = cell(1,4); xmang0(:)= {zeros(ln,ti0,tr,length(uStime))};
    ...
    for r = 1:numangplot
        ...
        if strcmp(num2str(ustime),cntrol)
            ...
            for d = 1:length(Dot)
                ...
                h2 = plotEllipses([x1m(dot) x2m(dot)],[x1std(dot) x2std(dot)]);
                h2.FaceColor = [stimcol(1,:) 0.3];
                h2.EdgeColor = colang(r,:);
                h2.LineWidth = 2;
            end
            %plot trajectory
            p1=plot(x1m,x2m,'.-');
            ...
            alphar = 0.1;  % Size of arrow head relative to the length of the vector
            betar  = 0.5;  % Width of the base of the arrow head relative to the length
        ...
        % plot arrow
        alphar = 0.1; %0.05  % Size of arrow head relative to the length of the vector
        betar  = 0.5; %0.4 % Width of the base of the arrow head relative to the length
        p4 = [x1m(a) x2m(a)]; % x(4): 4th x value point of a trajectory
        p5 = [x1m(b) x2m(b)]; % y(4): 4th y value point of a trajectory
        diff45 = p5 - p4;
        diff45 = diff45/norm(diff45); % normalize the vector length
...
if savefig
    saveas(h1,[figurepath 'subspaces/' 'FAtrajectories' nouSallanglab],'svg')
end
...
if savefig
    saveas(h2,[figurepath 'subspaces/' 'FAtrajectories_uSallang'],'svg')
end
