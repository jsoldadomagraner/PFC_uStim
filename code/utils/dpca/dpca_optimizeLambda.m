...
%     Xtrain = nanmean(XtrainFull, ndims(XtrainFull));
    % JOANA SM: changed to slow option to be able to handle nans in padded short trials
    Xtrain = nanmean(XtrainFull, 5);
...
%     XtestCen = bsxfun(@minus, Xtest, mean(Xtest(:,:),2));
    % JOANA SM: change to nansum since there are padded trials
    % with NaNs
    XtestCen = bsxfun(@minus, Xtest, nanmean(Xtest(:,:),2)); % JOANA SM
...
%                 recError = sum(sum((XtrainMargs{i} - V(:,whichMarg==i)*W(:,whichMarg==i)'*XtestCen(:,:)).^2));
                % JOANA SM: change to nansum since there are padded trials
                % with NaNs
                recError = nansum(nansum((XtrainMargs{i} - V(:,whichMarg==i)*W(:,whichMarg==i)'*XtestCen(:,:)).^2));
